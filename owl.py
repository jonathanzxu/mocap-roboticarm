...
class Context:
    ...
    def options(self, options=None):
        ...
        Supported options:
            ...
            event.info        0 or 1.  Enables or disables info events.
                              Default is 1.
            event.info.status 0 or 1.  Enable or disables devince info status events.
            event.????        0 or 1.  ???? equals a user-defined name for a filter.
                              Default is 0.
            system.LEDPower   Set LED duty cycle, range: 0.0 to 1.0
                              Initially set by value in system profile.
            system.profileChanged Force a profile reread.
...
Internal use only.
    ...
    def findTrackerInfo(self, type_name="", name=""):
        """
        Finds TrackerInfo instances that match the given filters.

        Arguments:
            type_name = The type of tracker, see createTrackers.
            name = The name of the tracker.

        Returns:
            A list of TrackerInfo instances with fields matching the arguments.

        Example:
            context.findTrackerInfo("rigid", "myrigidtracker2")
        """
        return filter(lambda t: \
                      (name == "" or t.name == name) and \
                      (type_name == "" or t.type == type_name), \
                      self.__trackers.itervalues())
    ...
    def assignMarkers(self, markerinfo=[]):
        ...
        Example:
            context.createTracker(0, "rigid")
            context.assignMarkers([MarkerInfo(1, 0, "mymarker1", "pos=100,0,0"),
                                   MarkerInfo(2, 0, "mymarker2", "pos=0,100,0")])
        """
    ...
    def deviceOptions(self, hw_id, device_options):
        """
        Set options for a device.

        Arguments:
            hw_id = The integer hardware identifier of the device
            device_options = a string of options, format is device specific

        Returns:
            True if successful, False on error

        Example:
            context.deviceOptions(0x2F3, "systemid=FFFFFFFF")
        """
        try:
            hw_id = "%x" % hw_id
            pass
            ...
            pass
        o = str.format("hwid=0x{} {}", hw_id, device_options)
        self.__log(str.format("send deviceoptions: {}", o));
        return self.__send(Type.BYTE, "deviceoptions", o);

    def findDeviceInfo(self, device_type, name = ""):
        """
        Find a DeviceInfo instance based on the given filters.

        Arguments:
            device_type = The type of the device.
                Accepted values: none, hub, camera, microdriver, rx
            name = The name of the device (example: "microdriver=1")

        Returns:
            A list of matching DeviceInfo instances.

        Example:
            context.findDeviceInfo("microdriver", "microdriver-1")
        """
        return filter(lambda d: \
                      (device_type == "" or device_type == d.type) and \
                      (name == "" or name == d.name), \
                      self.__devices.itervalues())
    ...
    def peekEvent(self, timeout_usec=0):
        ...
        """
        timeout = timeout_usec * 1E-6
        self.__recv(0 if self.__events else timeout)
    ...
    def nextEvent(self, timeout_usec=0):
        ...
        """
        timeout = timeout_usec * 1E-6
        self.__recv(0 if len(self.__events) else timeout)
        if not self.__events: return None
    ...
    def __recv(self, timeout):
        try:
            ...
            rsocks,wsocks,esocks = _select.select(rsocks, [], [], timeout)
    ...
    def __fatal_check(self, evt):
        if evt.type_id == Type.ERROR and (evt.name == "error" or evt.name == "fatal"):
            if "initializing" in self.__properties:
                raise InitError(evt.data)
            elif "opening" in self.__properties:
                raise OpenError(evt.data)
            pass
        pass
    ...
    def __process_event(self, evt):
        ...
        if evt.id == 0:
            self.__fatal_check(evt)
            if evt.type_id != Type.BYTE:
                _sys.stderr.write("warning: unknown event: type=%d id=%d" % (evt.type_id, evt.id))
                return 0
        ...
        elif evt.type_id == Type.CAMERA: self.__handle_camera(evt)
    ...
    def __send(self, _type, name, data):
        ...
        if _type == Type.BYTE:
            ...
            if _type == Type.FLOAT:
                ...
                t = int
                pass
            data = _struct.pack(form, *(t(d) for d in data))
            pass
    ...
    def __handle_internal(self, e):
        ...
        if data.startswith("table=types"):
            ...
            self.__properties["defaultprofile"] = _utils.tomap(data)["defaultprofile"]
            pass
        elif data.startswith("profiles.json"):
            self.__properties["profiles.json"] = _urlunquote(data)
            pass
    ...
    def __toggle_broadcast(self, v):
        if v == 3 and self.broadcast == None:
            self.broadcast = _socket.socket(_socket.AF_INET, _socket.SOCK_DGRAM, 0)
            self.broadcast.setsockopt(_socket.SOL_SOCKET, _socket.SO_REUSEADDR, 1)
            self.broadcast.setsockopt(_socket.SOL_SOCKET, _socket.SO_BROADCAST, 1)
            self.broadcast.setsockopt(_socket.SOL_SOCKET, _socket.SO_RCVBUF, self.ReceiveBufferSize)
            self.broadcast.setsockopt(_socket.SOL_SOCKET, _socket.SO_SNDBUF, self.SendBufferSize)
    ...
    def __handle_camera(self, e):
        if e.name == "cameras":
            self.__properties["cameras"] = e.data
            pass
        return
    ...
    def __parseDeviceInfo(table, options):
        ...
        for o in opts[1:]:
            try:
                ...
                v = o[0].split(',')
                if len(v) > 1:
                    hwid = _utils.str_to_int(v[0])
                    if len(v) == 4: #n, hwid, type, name
                        table[hwid] = DeviceInfo(_utils.str_to_int(v[1]), 0, v[2], v[3])
                        pass
                    pass
    ...
    def __wait(timeout, func):
        ...
        while True:
            if func(): return True
            if _time.time() - t0 >= timeout: break
            pass
        return False
    pass
...
class Scan:
    ...
    def send(self, message=b"python"):
        ...
        """
        if not self.socket:
            self.socket = _socket.socket(_socket.AF_INET, _socket.SOCK_DGRAM, 0)
            self.socket.setblocking(0)
            self.socket.setsockopt(_socket.SOL_SOCKET, _socket.SO_REUSEADDR, 1)
            self.socket.setsockopt(_socket.SOL_SOCKET, _socket.SO_BROADCAST, 1)
            pass
        message = message + '\0'
        if _sys.version_info.major >= 3: message = bytes(message, 'UTF-8')
...
class _utils:
    ...
    def getsocketerror(sock):
        return sock.getsockopt(_socket.SOL_SOCKET, _socket.SO_ERROR)
    @staticmethod
    def str_to_int(s):
        pfx = s[:2]
        if pfx == "0x":
...
class _Protocol:
    ...
    __struct_marker = _struct.Struct("<IIqffff")
    __struct_rigid = _struct.Struct("<IIq28sf")
    __struct_plane = _struct.Struct("<IIqHH16sf")
    __struct_plane2 = _struct.Struct("<ffff")
    __struct_camera = _struct.Struct("<II28sf")
    __struct_peak = _struct.Struct("<IIqHHIff")
    __struct_pose = _struct.Struct("<fffffff")

    class Header:
        __cksum = _struct.Struct("<BBBBBBBB")
        __hdr = _struct.Struct("<HBBIq")

        def __init__(self, buf=None):
            if buf: self.id, self.type, self.cksum, self.size, self.time = _Protocol.Header.__hdr.unpack_from(buf)
            else:
                self.id = -1
                self.type = -1
                self.cksum = 0
        ...
        def sum(self):
            ...
            for b in [self.id, self.type, self.cksum, self.size]:
                while b != 0:
                    s = _ctypes.c_uint8(s.value + (0xff & b))
                    b >>= 8
                    pass
                pass
            return s.value
        ...
        def pack(buf, id, _type, data):
            ...
            hdr = _Protocol.Header()
            hdr.id, hdr.type, hdr.cksum, hdr.size = id, _type, 0, len(data)
            _struct.pack_into("<HBBIq%ds" % len(data), buf, 0, id, _type, 256-hdr.sum(), len(data), 0, data)
            return 16 + len(data)
        pass

    def __init__(self, **kwargs):
        params = {"ReceiveBufferSize" : 4*1024*1024,
                  "SendBufferSize" : 1*1024*1024 }
        params.update(kwargs)
        self.inbuffer = bytearray(params["ReceiveBufferSize"])
        self.outbuffer = bytearray(params["SendBufferSize"])
        self.imv = memoryview(self.inbuffer)
        self.omv = memoryview(self.outbuffer)
        self.iposition = 0
        self.header = _Protocol.Header()
        self.hdr_size = 16
        pass

    def send(self, sock, id, _type, data):
        total_bytes = _Protocol.Header.pack(self.outbuffer, id, _type, data)
        sent_bytes = 0
        t0, dt, timeout = _time.time(), 0, 10
        assert(total_bytes > 0)
        while dt < timeout:
            # wait for socket ready
            rsocks, wsocks, esocks =_select.select([], [sock], [], 0)
            if len(wsocks) == 0: _time.sleep(0.001)
            else:
                ret = sock.send(self.omv[sent_bytes:total_bytes])
                if ret > 0:
                    sent_bytes += ret
                    t0 = _time.time()
                    # done
                ...
                pass
            pass
        raise SendError("socket timed out")
    ...
    def read_event(self):
        ...
        if not self.header.valid():
            _sys.stderr.write("read_event: invalid checksum!\n")
            return None
    ...
    def read_packet(self, sock):
        rsocks, wsocks, esocks = _select.select([sock], [], [sock], 0)
    ...
    def read_inputs(self, data):
        ...
        count = _struct.unpack_from("<I", data)
        o, inputs = 4, []
        while o < len(data):
            i = Input(*_struct.unpack_from("<QQqI", data, o))
            size = i.data
            o += 28
    ...
    def read_floats(self, data):
        return list(_struct.unpack_from("<%df" % (len(data)/4), data, 0))

    def read_ints(self, data):
        return list(_struct.unpack_from("<%di" % (len(data)/4), data, 0))

    def read_string(self, data):
        return _struct.unpack_from("<%ds" % (len(data)), data, 0)[0]
    pass

#
class _ProtocolUdp(_Protocol):
    def __init__(self, **kwargs):
        _Protocol.__init__(self, **kwargs)
        return
    def read_packet(self, sock):
        if len(_select.select([sock], [], [sock], 0)[0]) == 0:
            return 0
        self.iposition = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rx", dest="rx", action="store_const", const=1, default=0, help=argparse.SUPPRESS)
    ...
    parser.add_argument('tracker_files', metavar='tracker_files', type=str, nargs='*', help='zero or more .json files specifying a tracker')
    ...
    if args.device:
        try:
            ...
            if args.hub: OWL.option("event.hub", 1) # hub info events
            if args.rx: OWL.option("event.rx", 1) # rx info events

            #
            advanced_options = args.peaks or args.planes or args.inputs or args.hub or args.rx

            # create trackers, if any.
            # Note, trackers will overwrite each other if their ids conflict.
            if "json" in _sys.modules:
                for tf in args.tracker_files:
                    print("reading tracker file: " + tf)
                    tr = json.loads(open(tf, "rt").read())['trackers']
                    for t in tr:
                        OWL.createTracker(t["id"], str(t["type"]), str(t["name"]), str(t["options"]))
                        markers = [MarkerInfo(m["id"], t["id"], str(m["name"]), str(m["options"])) \
                                   for m in t["markers"]]
                        OWL.assignMarkers(markers)
                        pass
                    pass
                pass
            ...
            # poll for events
            event = None
            while event or (OWL.isOpen() and OWL.property("initialized")):
                ...
                if event.type_id == Type.FRAME:
                    ...
                    for camera in event.data:
                        print(camera)
                elif event.type_id == Type.DEVICEINFO:
                    print(event)
